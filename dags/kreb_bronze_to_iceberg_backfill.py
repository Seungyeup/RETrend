...
DAG_ID = "kreb_bronze_to_iceberg_backfill"
...
with DAG(
    ...
    catchup=False,
    max_active_runs=1,
    tags=["retrend", "kreb", "iceberg", "backfill"],
) as dag:
    spark_backfill = KubernetesPodOperator(
        task_id="spark_kreb_csv_to_iceberg_backfill",
        name="spark-kreb-csv-to-iceberg-backfill",
        namespace="airflow",
...
kind: ConfigMap
metadata:
  name: kreb-csv-to-iceberg-backfill-app
  namespace: kubeflow
data:
  kreb_csv_to_iceberg_backfill.py: |
    import os
    ...
    def build_spark():
        return (
            SparkSession.builder.appName("kreb-csv-to-iceberg-backfill")
            .config(
    ...
    def main():
        ...
        iceberg_table = os.environ.get("ICEBERG_TABLE", "iceberg.default.apt_trade")
        ...
        glob_path = f"{bronze}/LAWD_CD=*/DEAL_YM=*/page=*.csv"
        if not glob_has_matches(spark, glob_path):
            print(f"No input found: {glob_path}")
            spark.stop()
            return
...
kubectl -n kubeflow delete sparkapplication kreb-csv-to-iceberg-backfill --ignore-not-found

cat <<'YAML' | kubectl apply -f -
apiVersion: sparkoperator.k8s.io/v1beta2
kind: SparkApplication
metadata:
  name: kreb-csv-to-iceberg-backfill
  namespace: kubeflow
spec:
  type: Python
  mode: cluster
  ...
  mainApplicationFile: local:///opt/spark/app/kreb_csv_to_iceberg_backfill.py
  sparkVersion: 3.3.1
  restartPolicy:
    type: Never
  deps:
...
for i in $(seq 1 360); do
  state=$(kubectl -n kubeflow get sparkapplication kreb-csv-to-iceberg-backfill -o jsonpath='{.status.applicationState.state}' 2>/dev/null || true)
  ...
  if [ "$state" = "FAILED" ]; then
    ...
    kubectl -n kubeflow describe sparkapplication kreb-csv-to-iceberg-backfill || true
    exit 1
  fi
  sleep 10
done

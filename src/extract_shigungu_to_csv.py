...
# --- 로직 ---
def get_shigungu_list(cortar_no):
    """주어진 cortarNo에 대해 시군구 목록을 가져옵니다."""
...
if __name__ == "__main__":
    ...
    # 각 Sido에 대해 Shigungu 정보 수집
    for index, sido in sido_df.iterrows():
        sido_name = sido['cortarName']
        cortar_no = sido['cortarNo']
        
        print(f"\n[{sido_name}({cortar_no})]의 시군구 정보를 수집합니다...")
        
        shigungu_list = get_shigungu_list(cortar_no)
        
        if shigungu_list:
            # 부모 Sido 정보 추가
            for shigungu in shigungu_list:
                shigungu['parentCortarNo'] = cortar_no
                shigungu['parentCortarName'] = sido_name
            all_shigungu_list.extend(shigungu_list)
            print(f"  > {len(shigungu_list)}개의 시군구 정보를 수집했습니다.")
        else:
            print(f"  > 시군구 정보 수집에 실패했거나 데이터가 없습니다.")
            
        # 서버 부하를 줄이기 위한 지연
        time.sleep(0.5)

    if not all_shigungu_list:
        print("\n수집된 시군구 정보가 없습니다. 작업을 종료합니다.")
        exit()

...
# API URL 형식
base_url = 'https://new.land.naver.com/api/complexes/overview/{complexNo}?complexNo={complexNo}&tradeTypes='
...
def get_pyeonginfo(complex_no):
    """주어진 complexNo에 대해 평형 정보를 가져옵니다."""
    url = base_url.format(complexNo=complex_no)
    try:
        ...
        print(f"오류 발생 ({complex_no}): {e}")
        return None

if __name__ == "__main__":
    print("평형 정보 수집을 시작합니다.")

    # Complex 목록 파일 확인
    if not os.path.exists(complex_csv_path):
        print(f"오류: 단지 목록 파일이 없습니다. '{complex_csv_path}'")
        print("단지 정보 수집을 먼저 실행해주세요.")
        exit()

    # Complex 목록 읽기
    complex_df = pd.read_csv(complex_csv_path)
    print(f"'{complex_csv_path}' 파일에서 {len(complex_df)}개의 단지 정보를 읽었습니다.")

    all_pyeonginfo_list = []
    
    # 각 단지에 대해 평형 정보 수집
    for index, complex_item in complex_df.iterrows():
        complex_name = complex_item['complexName']
        complex_no = complex_item['complexNo']
        
        print(f"\n[{complex_item['eupmeandongCortarName']} > {complex_name}({complex_no})]의 평형 정보를 수집합니다...")
        
        pyeonginfo_data = get_pyeonginfo(complex_no)
        
        if pyeonginfo_data and 'pyeongs' in pyeonginfo_data:
            pyeongs = pyeonginfo_data['pyeongs']
            
            # 각 평형에 대해 정보 추가
            for pyeong in pyeongs:
                pyeong['complexNo'] = complex_no
                pyeong['complexName'] = complex_name
                pyeong['eupmeandongCortarName'] = complex_item['eupmeandongCortarName']
                
            all_pyeonginfo_list.extend(pyeongs)
            print(f"  > {len(pyeongs)}개의 평형 정보를 수집했습니다.")
        else:
            print(f"  > 평형 정보 수집에 실패했거나 데이터가 없습니다.")
            
        # 서버 부하를 줄이기 위한 지연
        time.sleep(0.5)

    if not all_pyeonginfo_list:
        print("\n수집된 평형 정보가 없습니다. 작업을 종료합니다.")
        exit()

    # 데이터프레임 생성 및 CSV 저장
    print(f"\n총 {len(all_pyeonginfo_list)}개의 평형 정보를 CSV 파일로 저장합니다.")
    pyeonginfo_df = pd.DataFrame(all_pyeonginfo_list)
    
    os.makedirs(output_dir, exist_ok=True)
    
    pyeonginfo_df.to_csv(pyeonginfo_csv_path, index=False, encoding='utf-8-sig')
    print(f"CSV 파일 저장 완료: {pyeonginfo_csv_path}") 
...
# Add app entrypoints into the image to avoid ConfigMap dependency.
RUN mkdir -p /opt/spark/app
COPY jobs/spark/kreb_csv_to_iceberg_incremental.py /opt/spark/app/kreb_csv_to_iceberg_incremental.py
